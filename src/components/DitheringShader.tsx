...
const DitheringShapes: Record<DitheringShape, number> = {
  simplex: 1,
  warp: 2,
  dots: 3,
  wave: 4,
...
const DitheringTypes: Record<DitheringType, number> = {
  random: 1,
  '2x2': 2,
  '4x4': 3,
  '8x8': 4,
...
export const DitheringShader: React.FC<ShaderProps> = ({
  width = 200,
  height = 200,
  colorBack = '#000000',
  colorFront = '#ffffff',
  ...
  useEffect(() => {
    ...
    const render = () => {
      const now = (Date.now() - startTimeRef.current) * 0.001 * speed;
      ...
      if (uniforms.u_time) context.uniform1f(uniforms.u_time, now);
      if (uniforms.u_resolution) context.uniform2f(uniforms.u_resolution, canvas.width, canvas.height);
      if (uniforms.u_colorBack) context.uniform4fv(uniforms.u_colorBack, hexToRgba(colorBack));
      if (uniforms.u_colorFront) context.uniform4fv(uniforms.u_colorFront, hexToRgba(colorFront));
      if (uniforms.u_shape) context.uniform1f(uniforms.u_shape, DitheringShapes[shape]);
      if (uniforms.u_type) context.uniform1f(uniforms.u_type, DitheringTypes[type]);
      if (uniforms.u_pxSize) context.uniform1f(uniforms.u_pxSize, pxSize);
